...
$j_opc(document).ready(function () {
	$j_opc(".radio-td label").click(function(){
		$j_opc(".radio-td").removeClass("active");
		$j_opc(this).parents('.radio-td').addClass("active");
	})
});
...
IWD.OPC = {
		...
		savePayment: function(){
			...
			if (payment.currentMethod != 'stripe') {
				...
				IWD.OPC.Checkout.xhr = $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/savePayment',form, IWD.OPC.preparePaymentResponse,'json');
		...
		preparePaymentResponse: function(response){
			...
			if (typeof(response.review)!= "undefined"){
				...
				IWD.OPC.Checkout.removePriceAndQty();
		...
		saveCustomerComment: function(){
			$j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/comment',{"comment": $j_opc('#customer_comment').val()});
		}, 
...
IWD.OPC.Checkout = {
		...
		init:function(){		
			...
			IWD.OPC.Checkout.saveOrderUrl = IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/saveOrder',
			this.success = IWD.OPC.Checkout.config.baseUrl + 'checkout/onepage/success',
			
			//DECORATE
			this.clearOnChange();
			this.removePriceAndQty();			
...
//				IWD.OPC.recheckAgree();
		...
		pullReview: function(){
			IWD.OPC.Checkout.lockPlaceOrder();
			IWD.OPC.Checkout.xhr = $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/review',function(response){
				IWD.OPC.Checkout.xhr = null;
				IWD.OPC.Checkout.hideLoader();
				IWD.OPC.Checkout.unlockPlaceOrder();
				if (typeof(response.review)!="undefined"){
...
//					IWD.OPC.recheckAgree();
		...
		pullPayments: function(){
			IWD.OPC.Checkout.lockPlaceOrder();
			IWD.OPC.Checkout.xhr = $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/payments',function(response){
				IWD.OPC.Checkout.xhr = null;
		...
		reloadShippingsPayments: function(form_type, delay){
			...
			IWD.OPC.Checkout.updateShippingPaymentProgress = setTimeout(function(){
				...
				IWD.OPC.Checkout.xhr2 = $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/reloadShippingsPayments',form, IWD.OPC.Checkout.reloadShippingsPaymentsResponse,'json');
...
IWD.OPC.Website = {
	init: function() {
		this.initChangeWebsite();
	},
	initChangeWebsite: function(){
		$j_opc('#opc-website-form #username').blur(function(){
			IWD.OPC.Website.validateForm();
		});
	},
	validateForm: function(){
		var valid = IWD.OPC.Website.validateWebsiteForm();
		if(!valid)
		{
			IWD.OPC.Website.check();
		}
	},
	validateWebsiteForm: function() {
		// check all required fields not empty
		var is_empty = false;
		if($j_opc('#opc-website-form #username').val() == '' && $j_opc('#opc-website-form #username').css('display') != 'none' && !$j_opc('#opc-website-form #username').attr('disabled'))
		{
			is_empty = true;
		}
		return is_empty;
	...
	check: function() {
		...
		IWD.OPC.Checkout.ajaxProgress = setTimeout(function(){
				var form = $j_opc('#opc-website-form').serializeArray();
				...
				IWD.OPC.Checkout.xhr = $j_opc.post(
					IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/checkWebsite',
					form, 
					function(response) {
						IWD.OPC.Checkout.xhr = null;

						$j_opc('#availablity_website').html(response.message);

						if (typeof(response.error) != "undefined" && response.error == true) {
							$j_opc('#opc-website-form #username').addClass('validated');
							$j_opc('#availablity_website').addClass('error');
						}
						else
						{
							$j_opc('#opc-website-form #username').removeClass('validated');
							$j_opc('#availablity_website').removeClass('error');
						}
...
IWD.OPC.Profile = {
	...
	initChangeProfile: function(){
		$j_opc('#opc-ambassador-profile select').change(function(){
			IWD.OPC.Profile.validateForm();
		});
	},
	validateForm: function(){
		var valid = IWD.OPC.Profile.validateProfileForm();
		if(!valid)
		{
			var addressForm = new VarienForm('opc-ambassador-profile');
			if (addressForm.validator.validate()) {
				IWD.OPC.Profile.save();
			}
		}
	},
	validateProfileForm: function() {
		// check all required fields not empty
		var is_empty = false;
		$j_opc('#opc-ambassador-profile .required-entry').each(function(){
			if($j_opc(this).val() == '' && $j_opc(this).css('display') != 'none' && !$j_opc(this).attr('disabled'))
			{
				is_empty = true;
			}
	...
	save: function() {
		...
		IWD.OPC.Checkout.ajaxProgress = setTimeout(function(){
				var form = $j_opc('#opc-ambassador-profile').serializeArray();
				...
				IWD.OPC.Checkout.xhr = $j_opc.post(
					IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/saveProfile',
					form, 
...
IWD.OPC.StarterKit = {
	...
	initChangeProfile: function(){
		$j_opc('#co-starterkit-form input[type=radio]').click(function(){
			IWD.OPC.StarterKit.validateForm();
		});
	},
	validateForm: function(){
		var addressForm = new VarienForm('co-starterkit-form');
		if (addressForm.validator.validate()) {
			IWD.OPC.StarterKit.save();
		}
	},
	save: function() {
		if (IWD.OPC.Checkout.ajaxProgress!=false){
		...
		IWD.OPC.Checkout.ajaxProgress = setTimeout(function(){
				var form = $j_opc('#co-starterkit-form').serializeArray();
...
IWD.OPC.Billing = {
		...
		validateAddressForm: function(form){
			...
			$j_opc('#opc-address-form-billing .required-entry').each(function(){
				...
				if($j_opc(this).attr('type') == "checkbox" && $j_opc(this).prop('checked') == false)
					is_empty = true;
			});
			if(is_empty)
				return false;
		...
		save: function(){
			...
			IWD.OPC.Checkout.ajaxProgress = setTimeout(function(){
					...
					IWD.OPC.Checkout.xhr = $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/saveBilling',form, IWD.OPC.Checkout.prepareAddressResponse,'json');
			}, 500);
		},
...
IWD.OPC.Shipping = {
		...
		save: function(){
			...
			IWD.OPC.Checkout.ajaxProgress = setTimeout(function(){
					...
					IWD.OPC.Checkout.xhr = $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/saveShipping',form, IWD.OPC.Checkout.prepareAddressResponse,'json');
			}, 500);
		},
		...
		saveShippingMethod: function(update_payments, reload_totals){
			...
			IWD.OPC.Checkout.ajaxProgress = setTimeout(function(){
				...
				IWD.OPC.Checkout.showLoader();
				IWD.OPC.Checkout.xhr = $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/saveShippingMethod',form, IWD.OPC.Checkout.prepareShippingMethodResponse);
			}, 600);
		},
...
IWD.OPC.Coupon = {
		...
		applyCoupon: function(remove){
			...
			IWD.OPC.Checkout.showLoader();
			IWD.OPC.Checkout.xhr = $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'ambassador/coupon/couponPost',form, IWD.OPC.Coupon.prepareResponse,'json');
		},
...
IWD.OPC.Login ={
		...
		init: function(){
			...
			$j_opc('#forgotpassword-button-set .btn').click(function(){
				...
				IWD.OPC.Checkout.showLoader();
				IWD.OPC.Checkout.xhr = $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'ambassador/json/forgotpassword',form, IWD.OPC.Login.prepareResponse,'json');
			});
...
IWD.OPC.Decorator = {
		...
		updateGrandTotal: function(response){
			$j_opc('h5.grand_total span').html(response.grandTotal);
