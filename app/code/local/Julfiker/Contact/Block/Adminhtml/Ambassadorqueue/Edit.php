...
class Julfiker_Contact_Block_Adminhtml_Contact_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        $this->_updateButton(
            'save',
            'label',
            Mage::helper('julfiker_contact')->__('Save queue')
        );
        $this->_updateButton(
            'delete',
            'label',
            Mage::helper('julfiker_contact')->__('Delete queue')
        );
        $this->_addButton(
            'saveandcontinue',
            array(
    ...
    {
        if (Mage::registry('current_contact') && Mage::registry('current_ambassadorqueue')->getId()) {
            return Mage::helper('julfiker_contact')->__(
                "Edit Contact '%s'",
                $this->escapeHtml(Mage::registry('current_ambassadorqueue')->getContactStatus())
            );
        } else {
            return Mage::helper('julfiker_contact')->__('Add Queue');
        }
    }
}
