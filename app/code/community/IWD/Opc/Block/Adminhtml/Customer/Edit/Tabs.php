...
class IWD_Opc_Block_Adminhtml_Customer_Edit_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
{
...
*/
        ...
        if (Mage::registry('current_customer')->getId()) {
            ...
            $addressCollection = Mage::getModel('opc/address')
                ->getCollection()->addFieldToFilter('customer_id', Mage::registry('current_customer')->getId());

            if($addressCollection->count()) {
                $this->addTab('ambassador', array(
                    'label'     => Mage::helper('customer')->__('Ambassador Address'),
                    'content'   => $this->getLayout()->createBlock('opc/adminhtml_customer_edit_tab_ambassador')->toHtml(),
                ));
            }
        }
